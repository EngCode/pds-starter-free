...
<div class="px-page-loader" style="position: fixed; top:0; left:0; z-index: 999999999; width: 100vw; height: 100vh; background-color: <?php if(get_option('pds-loading-bg')) { echo get_option('pds-loading-bg'); } else { echo 'var(--body-bg)'; }; ?>; text-align: center;">
    ...
    <div class="content-box" style="position: absolute; top: 50%; left: 50%; transform: translateX(-50%) translateY(-50%)">
        <!-- Spinner -->
        <img class="spinner" style="opacity: 0.5" width="42" height="42" src="<?php if(get_option('pds-loading-image')) { echo get_option('pds-loading-image'); } else { echo get_template_directory_uri().'/assets/img/loading.svg'; }; ?>" alt="<?php echo __('Loading', 'phenix');?>" />
        <!-- Paragraph -->
...
<script defer>
    ...
    window.addEventListener('load', loaded => Phenix(".px-page-loader").fadeOut(500, 300));
    //===> When Leaving Page <===//
